...
int					g_saving = 1;
void				*g_buffer;
char 				*g_capture_name = "Universal Audio";
t_wav				g_header;
...
init(void)
{
	...
	/* WARNING:sets the global var for the capture and output logical dev */

	SDL_AudioSpec a_capture_spec = set_capture_device("Universal Audio");
	SDL_AudioSpec a_output_spec = set_output_device("Universal Audio");

	/* same here stream is set in global g_inst.stream */
	stream_capture_init(a_capture_spec, g_inst.cDevID);
