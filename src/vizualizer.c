...
make_plot(size_t i, float c_w, float c_h, float n_rect, float fft_value, float rect_w, float space, int way)
{
	float x1 = 0;
	float y1 = 0;
	float w1 = 0;
...
apply_fft(Uint8 *dst, Uint8 *src, Uint32 length, Audio_wave *wave, float adjust)
{
	...
	TracyCZoneNC(b, "fftw", 0x00FF00, 1);
	m = fft(NULL, length);
	TracyCZoneEnd(b);
